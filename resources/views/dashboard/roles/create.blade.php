    ...
    <div class="col-xl-10 mx-auto">
        <div class="card">
            <div class="card-header tx-medium bd-0 tx-white bg-primary-gradient">
                <i class="fas fa-plus mr-2"></i> Ajouter Role
            </div>
            <div class="card-body">
                        <form class="form-horizontal" action="{{ route('dashboard.role.store') }}" method="post">
                            @method('Post')
                             @csrf     
                             <div class="form-group ">
                                <div class="row">
                                    ...
                                    <div class="col-md-9">
                                        <input type="text" name="name"class="form-control @error('name') parsley-error @enderror" placeholder="Nom Role" value={{ old('name') }}>
                                        @error('name')
                                    ...
                                    <div class="col-md-9">
                                       {{--   <div class="custom-controls-stacked ">
                                        ...
                                        <select multiple="multiple" name="permissions[]"class=" form-control select2 @error('permissions')  parsley-error @enderror">
											@foreach($permissions as $permission)
                                            <option value="{{$permission->id}}" {{ (is_array(old('permissions')) and in_array($permission->id, old('permissions'))) ? ' selected' : '' }}>{{$permission->name}}</option>
											@endforeach
										</select>
